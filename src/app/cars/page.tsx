...
export default async function Page() {
  ...
  return (
    <main>
      <PageTitle>You have {cars.length} Cars</PageTitle>
...
const Car = ({ car }: { car: Car }) => {
  ...
  return (
    <li key={car.id}>
      <Link href={`/cars/${car.id}`} className="grid border border-gray-200 hover:border-blue-600 transition duration-150 ease-in pb-3 leading-tight rounded-md overflow-hidden hover:scale-[1.01]">
        <Image
          src={`/cars/${car.image}`}
